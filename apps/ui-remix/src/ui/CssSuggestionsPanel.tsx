...
const closeBtn = css({
  ...
  placeItems: 'center',
  fontSize: '16px',
  '&:hover': { color: 'var(--fg)', borderColor: 'var(--fg-faint)' },
});
...
const devCell = (covered: boolean) =>
  css({
    ...
    textAlign: 'center',
    background: covered
      ? 'color-mix(in oklab, var(--ok, #4ade80) 16%, transparent)'
      : 'color-mix(in oklab, var(--warn, #f5a623) 18%, transparent)',
    ...
    return (
      <aside mix={drawer} role="region" aria-label="CSS suggestions">
        <div mix={head}>
          ...
          <button type="button" mix={[closeBtn, on('click', close)]} aria-label="Collapse panel">
            ×
          </button>
        </div>
        <div mix={body}>
          <div mix={stats}>
            ...
            {styles.containerQueries ? (
              <span mix={stat}>{styles.containerQueries} @container</span>
            ) : null}
          </div>
          {styles.paradigms.length ? (
            <div mix={para}>
              {styles.paradigms.map((p) => (
                <span key={p} mix={paraChip}>
                  {p}
                </span>
              ))}
            </div>
            ...
            {styles.devices.map((d) => (
              <div key={d.name} mix={devCell(d.covered)} title={`${d.minPx}–${d.maxPx ?? '∞'}px`}>
                ...
                <div
                  mix={[
                    devState,
                    css({ color: d.covered ? 'var(--ok, #4ade80)' : 'var(--warn, #f5a623)' }),
                  ]}
                >
                  {d.covered ? '✓' : 'gap'}
                </div>
          ...
          {count === 0 ? (
            <p mix={clean}>
              ✓ No CSS issues detected. Naming, specificity, responsiveness, and tooling look
              healthy.
            </p>
          ) : (
            findings.map((f) => (
              <div key={f.id} mix={card}>
                ...
                {f.suggestion ? (
                  <div mix={snipWrap}>
                    ...
                    <button
                      type="button"
                      mix={[copyBtn, on('click', () => copy(f.suggestion ?? ''))]}
                    >
                      copy
                    </button>
                  </div>
        ...
        <button type="button" mix={[closeTab, on('click', close)]} aria-label="Close panel">
          <span aria-hidden="true" mix={closeTabIcon}>
            ›
          </span>
          <span>Close</span>
        </button>
