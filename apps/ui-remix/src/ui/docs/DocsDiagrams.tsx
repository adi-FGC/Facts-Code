  ...
  return () => {
    ...
    if (all.length === 0) {
      return (
        <div mix={page}>
          <p mix={empty}>
            No diagrams found. Fenced <code>mermaid</code> / <code>plantuml</code> /{' '}
            <code>dot</code> blocks in any doc show up here — including entity-relationship
            diagrams.
          </p>
        </div>
      );
    ...
    const shown: DiagramRow[] =
      filter === 'all' ? all : all.filter((d) => diagramTypeLabel(d) === filter);
        ...
        <div mix={filters}>
          {types.map((t) => (
            <button
              key={t}
              type="button"
              mix={[fChip(filter === t), on('click', () => setFilter(t))]}
            >
              {t}
              {t !== 'all'
                ? ` (${all.filter((d) => diagramTypeLabel(d) === t).length})`
                : ` (${all.length})`}
            </button>
          ))}
        </div>
        {shown.map((d, i) => (
          <div key={i} mix={card}>
            <div mix={cardHead}>
              ...
              <button type="button" mix={[copyBtn, on('click', () => copy(d.code))]}>
                copy
              </button>
            </div>
            <pre mix={code}>
