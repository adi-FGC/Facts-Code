...
const items = css({
  padding: '0 var(--space-5) var(--space-4) calc(var(--space-5) + var(--space-4))',
});
const item = css({
  display: 'grid',
  ...
  return () => {
    ...
    if (roadmaps.length === 0) {
      return (
        <div mix={page}>
          <p mix={empty}>
            No checklists or roadmaps found in the docs. Add <code>- [ ] task</code> items to a doc
            and they'll appear here as a verifiable roadmap.
          </p>
        </div>
      );
    }
    return (
      <div mix={page}>
        ...
        <p mix={lede}>
          Inferred from every checklist in the docs. Progress bars are computed from the actual{' '}
          <code>- [x]</code> state — so this is a verifiable view, not a hand-maintained one.
        </p>
        {roadmaps.map((r) => {
              ...
              {r.sections.map((s, si) => {
                ...
                return (
                  <div key={key}>
                    <button
                      type="button"
                      mix={[secBtn, on('click', () => toggle(key))]}
                      aria-expanded={open ? 'true' : 'false'}
                    >
                      <span aria-hidden="true" mix={caret(open)}>
                        ▸
                      </span>
                      <span mix={secTitle}>{s.title}</span>
                      <span mix={miniBar}>
                        <span mix={miniFill(spct)} />
                      </span>
                      <span mix={secCount}>
                        {s.done}/{s.total}
                      </span>
                    ...
                    {open ? (
                      <div mix={items}>
                        {s.items.map((it, i) => (
                          <div key={i} mix={item}>
                            <span aria-hidden="true" mix={it.done ? boxOk : boxOpen}>
                              {it.done ? '☑' : '☐'}
                            </span>
