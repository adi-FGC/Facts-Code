...
export function DocsTodos(handle: Handle<{ data: Dataset }>) {
  ...
  return () => {
    ...
    if (sum.buckets.length === 0) {
      return (
        <div mix={page}>
          <p mix={empty}>No todos or checklists found in the project's docs.</p>
        </div>
      );
    }
    ...
    return (
      <div mix={page}>
        ...
        <div mix={statRow}>
          <span>
            <span mix={big}>
              {sum.totalDone}/{sum.totalCheckboxes}
            </span>{' '}
            <span mix={lbl}>tasks done</span>
          </span>
          <span>
            <span mix={big}>{pct}%</span> <span mix={lbl}>complete</span>
          </span>
          <span>
            <span mix={big}>{sum.totalMarkers}</span> <span mix={lbl}>TODO/FIXME markers</span>
          </span>
          <button type="button" mix={[toggle, on('click', flip)]}>
          ...
          return (
            <div key={b.docPath} mix={card}>
              <div mix={cardHead}>
                ...
                <span mix={cardPath}>
                  {b.done}/{b.checkboxes} done{b.markers ? ` · ${b.markers} markers` : ''}
                </span>
              </div>
              {items.map((t, i) => (
                <div key={i} mix={item}>
                  <span
                    aria-hidden="true"
                    mix={t.done === null ? markerTag : t.done ? boxOk : boxOpen}
                  >
                    {t.done === null ? '⚑' : t.done ? '☑' : '☐'}
                  </span>
