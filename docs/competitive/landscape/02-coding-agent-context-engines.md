...
- No tool produces a **pre-assembled, agent-agnostic context payload** in a standardized compact wire format that _any_ agent can consume — every engine is coupled to one agent's prompt format.
