...
explicitly resisting bloat in a tool whose moat is _clarity_.
...
| C25 | Architecture sketch           | Detect layered patterns; draw what code _does_ vs README claims.           |
...
| C29 | Stale tests panel             | Tests for code that hasn't changed in N months _and_ hasn't run in CI.     |
...
- **DROP** — explicit decision _not_ to build, with rationale.
