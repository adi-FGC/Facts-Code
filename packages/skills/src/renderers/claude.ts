...
function renderSkillMd(spec: SkillSpec, slug: string): string {
  ...
  if (spec.keyFiles.length) {
    ...
    sections.push('');
    sections.push('Most-imported files — read these first to understand the project surface:');
    sections.push('');
  ...
  if (spec.openRisks.length) {
    ...
    sections.push('');
    sections.push(`Call \`list_risks\` for the full risk surface (this list is capped).`);
  }
  ...
  sections.push(`<!-- factstack ${spec.factsVersion} · generated ${spec.generatedAt} -->`);
