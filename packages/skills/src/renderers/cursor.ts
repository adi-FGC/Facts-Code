...
function renderCursorRules(spec: SkillSpec): string {
  ...
  const stack = spec.languages.length ? `${spec.languages[0]!.id}` : 'this';
  const frameworks = spec.frameworks.length ? ` (${spec.frameworks.slice(0, 4).join(', ')})` : '';
  lines.push(`You are working in the **${spec.name}** codebase — a ${stack}${frameworks} project.`);
  if (spec.intent) {
    lines.push('');
  ...
  if (spec.keyFiles.length) {
    ...
    lines.push('');
    lines.push('Hub files in the import graph (read before answering structural questions):');
    lines.push('');
  ...
  lines.push(`<!-- Refresh: \`factstack export-skills --target cursor\` -->`);
