...
export const MCP_TOOL_CATALOG: readonly McpToolMeta[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'Target file path or symbol id (path#name@line); source endpoint for path-between.',
        },
        ...
        depth: {
          type: 'number',
          description:
            'Transitive depth. Default 1 for most verbs; 3 for `impact` (blast radius) when omitted.',
        },
        minConfidence: {
          type: 'string',
          enum: ['extracted', 'inferred', 'ambiguous'],
          description: 'Keep only edges at least this certain.',
        },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: {
          type: 'object',
          description: 'Structured GraphQuery (start/traverse/where/select/limit).',
        },
        format: { type: 'string', enum: ['pack', 'json'], default: 'pack' },
      },
    },
  },
  {
    ...
    description:
      'Return the symbol outline (declarations) for a single file, computed live from the source, plus a `refs` table of outgoing symbol-graph edges with confidence (populated when the project was analyzed with --symbols). Returns FactsPack by default; pass format:"json" for the legacy JSON shape.',
    inputSchema: {
      type: 'object',
      required: ['path'],
      properties: {
  ...
  },
  {
    ...
    description:
      'List scanner findings, optionally filtered by severity or category. Returns FactsPack by default; pass format:"json" for the legacy JSON shape.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Read .facts/MEMORY.md — a compact (2-10 KB) markdown brief that summarizes the project for AI agents. ALWAYS call this first when joining a new project; it replaces a 40-200 KB cold-read of agent.json.',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        timestamp: {
          type: 'string',
          description: 'ISO 8601 timestamp lower bound, e.g. "2026-04-30T00:00:00Z".',
        },
      },
    },
  },
  ...
  },
  {
    ...
    description:
      'Filter .facts/learnings.jsonl by since/until/agent/outcome/action/tag. Returns most-recent-first, capped at 5000 events. Use for "what has this codebase\'s agents been right about?" calibration questions. Returns FactsPack by default; pass format:"json" for the legacy JSON shape.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'List every environment variable read by the codebase, with read sites + captured defaults. Sorted by read count desc. Empty when no env reads are detected. Returns FactsPack by default (one row per read site); pass format:"json" for the legacy JSON shape (one entry per name, with reads[] nested).',
    inputSchema: {
      type: 'object',
      properties: {
        format: { type: 'string', enum: ['pack', 'json'], default: 'pack' },
  ...
  },
  {
    ...
    description:
      'List leaked-credential findings from the secrets scanner — all `risks` entries with category === "secret". Each finding includes ruleId, file, line, severity, and a redacted preview (raw secrets are NEVER emitted; enforced at the type level in @factstack/scanners). JSON-only response.',
    inputSchema: {
      type: 'object',
      properties: {
        severity: { type: 'string', enum: ['info', 'low', 'medium', 'high', 'critical'] },
  ...
  },
  {
    ...
    description:
      "List known CVE/GHSA advisories matched against the project's dependency manifests. Returns {findings, scan, scanAgeDays, stale, lastChecked, manifestCount}. `scan` carries the last scan's metadata (scannedAt, packagesQueried, findings) — scan:null means never scanned; scan present with findings:0 means scanned-and-clean. When `stale` is true (scan older than 7 days) or scan is null, pass refresh:true to UPDATE the list: it re-queries OSV.dev live and persists the result into .facts/ (survives future re-analyzes). Refresh is the only network call; plain listing reads the artifact. Filterable by severity / ecosystem / package name.",
    inputSchema: {
      type: 'object',
      properties: {
        ...
        ecosystem: {
          type: 'string',
          enum: ['npm', 'pypi', 'cargo', 'go', 'maven', 'rubygems', 'unknown'],
        },
        package: {
          type: 'string',
          description: 'Filter to advisories affecting this exact package name.',
        },
        refresh: {
          type: 'boolean',
          description:
            'Re-query OSV.dev live and persist the updated list + scan metadata before returning (the update mechanism). Default false (artifact read only).',
          default: false,
        },
      },
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        focus: {
          type: 'string',
          description: 'Project-relative file path; required when view=focal.',
        },
        depth: {
          type: 'number',
          description: 'Max BFS depth for focal view (default 2, max 5).',
          default: 2,
        },
      },
    },
  ...
  },
  {
    ...
    description:
      'Estimate the AI-context token cost of a project file (by `path`) or a raw `text` snippet. For a `path` already in the analyzed artifact this returns the EXACT pre-computed tokenCost; otherwise it live-reads + estimates (char-based cl100k approximation, within ~8% of tiktoken). Answers "does this fit in context?" / "how much will reading this cost?". JSON response. Exactly one of path/text.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'The coding task in plain words, e.g. "add a role field to User".',
        },
        seeds: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional explicit seed file paths or symbol ids to anchor on.',
        },
        budgetTokens: {
          type: 'number',
        ...
        maxHops: {
          type: 'number',
          description: 'Graph expansion radius from the seeds (default 2).',
          default: 2,
        },
        format: {
          type: 'string',
          enum: ['pack', 'json'],
          description: 'Response shape (default pack).',
          default: 'pack',
        },
      },
      required: ['query'],
    ...
    inputSchema: {
      ...
      properties: {
        have: {
          type: 'string',
          description:
            'The 12-hex sha256 of the agent.pack master you currently hold (from a prior sync_pack `sha` / the pack trailer). Omit on first fetch.',
        },
      },
    },
  },
...
export function mcpCatalogMatchesNames(): boolean {
  ...
  return names.length === MCP_TOOL_NAMES.length && names.every((n, i) => n === MCP_TOOL_NAMES[i]);
}
